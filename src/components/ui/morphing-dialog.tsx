...
interface MorphingDialogTriggerProps {
  ...
  /**
   * Called when the trigger is activated, BEFORE the dialog opens. May be
   * async — the dialog opens after this resolves. Return `false` to abort
   * opening (e.g. when there's nothing to show). Use it to prepare modal
   * content (e.g. resolve media entries) so the morph transition animates
   * into the fully-rendered dialog.
   */
  onOpen?: (triggerEl: HTMLElement | null) => void | Promise<void | boolean>
}
...
export function MorphingDialogTrigger({
  ...
  const handleClick = React.useCallback(async () => {
    if (triggerRef.current) {
      const shouldOpen = await onOpen?.(triggerRef.current)
...
export function MorphingDialogContainer({
  ...
  return (
    <dialog
      ...
      className={cn(
        // Re-enable pointer events explicitly: when a MorphingDialog is used
        // inside a Radix modal (e.g. the media lightbox inside the editor
        // Sheet), Radix freezes the body with `pointer-events: none` and only
        // re-enables its own content. A native <dialog> in the top layer
        // inherits `none` and would otherwise swallow no clicks at all.
        "pointer-events-auto",
